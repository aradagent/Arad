...
@media only screen and (device-width: 375px) and (device-height: 812px) and (-webkit-device-pixel-ratio: 3),
    ...
    (function(){
      ...
      var LAYER_SELECTOR = [
        ...
        '#slideEditorModal', '#qaEditorModal',
        '.ava-prof-drawer', '#friendDetailsModal', '[id^="rejectModal_"]'
        // توجه: .notification-overlay/.notification-dropdown عمداً از این لیست حذف شدند.
        // این دو در notif.css یک z-index ثابت و !important (9200) دارند، پس نوشتنِ
        // z-index اینلاین رویشان اصلاً اثری ندارد (!important در استایل‌شیت همیشه از
        // استایل اینلاینِ معمولی برنده می‌شود) — فقط باعث می‌شد هر بار که لیست
        // اعلان‌ها رفرش می‌شود (هر ۵ ثانیه، وقتی دراپ‌داون باز است)، MutationObserver
        // زیرِ درختِ کاملِ آیتم‌های تازه‌ساخته‌شده را برای این سلکتورها اسکن کند —
        // هزینه‌ای واقعی و بی‌فایده که با باز/بسته‌شدنِ مکررِ زنگوله سنگین‌تر هم می‌شد
        // و می‌توانست حس «هنگ‌کردن» بدهد.
      ].join(',');
